...
class Message
{
    ...
    public function parse(): bool
    {
        ...
        $element_limit = 30000;
        if (2 * $element_limit < substr_count($this->message, '<')) {
            return false;
        }
    ...
    public function tagClose(\XMLParser $parser, string $tag): void
    {
        ...
        switch ($tag) {
            ...
            case 'value':
                ...
                break;
            case 'boolean':
                $value = boolval($this->_currentTagContents);
                $valueFlag = true;
                break;
            case 'base64':
        ...
        if ($valueFlag) {
            $arrayStructsCount = \count($this->_arraystructs);
            if ($arrayStructsCount > 0) {
                // Add value to struct or array
                if ($this->_arraystructstypes[\count($this->_arraystructstypes) - 1] === 'struct') {
                    // Add to struct
                    $this->_arraystructs[$arrayStructsCount - 1][$this->_currentStructName[\count($this->_currentStructName) - 1]] = $value;
                } else {
                    // Add to array
                    $this->_arraystructs[$arrayStructsCount - 1][] = $value;
                }
            } else {
